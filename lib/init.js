...
const { successInitLog } = require("./utils/customLogLabel");
const { clearHelper } = require("./utils");
...
function go() {
  next
      ...
      return inquirer
        .prompt([
          ...
          },
          {
            ...
            message: "项目的简介",
            default: `A normal ${templateName} project`
          }
        ])
        .then(answers => {
          return {
    ...
    .then(context => {
      generator(
        context.metadata,
        context.downloadTemp + "/template",
        context.name
      ).then(() => {
        console.log(successInitLog(templateName, context.root));
      });
    })
    .catch(err => {
